...
function movingAverageRecovery($data, $p) {
    ...
    list($firstSlope, $firstIntercept) = linearRegression(range(1, count($firstValues)), $firstValues);
    list($lastSlope, $lastIntercept) = linearRegression(range(1, count($lastValues)), $lastValues);
...
// Линейная аппроксимация для прогнозирования
function forecastNextValues($data, $moving_averages, $name, $months_ahead = 2) {
    $count = count($moving_averages);
    $x = range(1, $count);
    $y = array_column($moving_averages, $name);
    list($slope, $intercept) = linearRegression($x, $y);
    
    $predictions = [];
    for ($i = 1; $i <= $months_ahead; $i++) {
        $predictions[] = linearRegressionPredict($count + $i, $slope, $intercept);
    }
    return $predictions;
}

// Прогнозные значения
$next_month_index = count($listens_data);
$last_month = $listens_data[$next_month_index - 1]['month'];
$next_months = [];
for ($i = 1; $i <= 2; $i++) {
    $next_months[] = date('Y-m', strtotime("+$i month", strtotime($last_month)));
}

$next_moving_average_3 = forecastNextValues($listens_data, $moving_averages[3], 'moving_average');
$next_weighted_average = forecastNextValues($listens_data, $weighted_moving_averages, 'weighted_moving_average');
$next_moving_average_7 = forecastNextValues($listens_data, $moving_averages[7], 'moving_average');

// Обновляем данные на 2 следующих месяца
for ($i = 0; $i < 2; $i++) {
    $listens_data[] = ['month' => $next_months[$i], 'count' => '—'];
    $moving_averages[3][] = ['month' => $next_months[$i], 'moving_average' => $next_moving_average_3[$i]];
    $weighted_moving_averages[] = ['month' => $next_months[$i], 'weighted_moving_average' => $next_weighted_average[$i]];
    $moving_averages[7][] = ['month' => $next_months[$i], 'moving_average' => $next_moving_average_7[$i]];
}
?>
